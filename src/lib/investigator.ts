 ...
 *
 * Después de la primera query del agente, evalúa si los resultados tienen una
 * anomalía clara que valga la pena investigar (caída drástica, outlier,
 * concentración inesperada). Si la hay, propone UNA query de seguimiento
 * SQL-readonly que ayude a entender la causa raíz.
 *
 * Diseño:
 *   - Costo controlado: una sola llamada extra a Claude con max_tokens bajo
 *   - Solo se activa si el primer resultado tiene datos y es de query_database
 *   - Devuelve null si no detecta nada notable (caso común — no fuerza follow-up)
...
export interface FollowUpProposal {
    ...
    question: string;
    /** SQL T-SQL read-only que responde la pregunta */
    sql: string;
    /** Una oración explicando por qué este follow-up importa */
    rationale: string;
}

/**
 * Decide si vale la pena investigar más y propone la query de seguimiento.
 * Retorna null si los datos no muestran nada notable o si la heurística
...
export async function proposeFollowUp(opts: {
    ...
    // No vale la pena investigar resultados triviales (1 fila × 1 columna)
...
Si decides investigar, debes generar UNA consulta T-SQL adicional que:
• Sea SOLO SELECT (read-only, sin INSERT/UPDATE/DELETE)
• Profundice en la dimensión anómala (ej: si hay caída en Sur → desglose
  por hora/depto/cajero EN Sur específicamente)
• Use exactamente los nombres de tablas/columnas del schema
• Sea concisa, no más de 100 líneas

ESQUEMA DISPONIBLE (parcial, solo referencia):
