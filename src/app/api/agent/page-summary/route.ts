 ...
 *
 * Recibe el snapshot completo de un dashboard (KPIs visibles, distribuciones,
 * top items) y genera un párrafo narrativo corto al estilo editorial.
 *
 * Body: {
 *   pageContext: string,              // ej: "Dashboard de Ventas por Cliente"
 *   period: { fechaInicio, fechaFin },
 *   scope: string,                    // ej: "5 sucursales seleccionadas"
 *   kpis: Record<string, any>,        // ej: { ContadoMonto: 84700, ... }
 *   highlights?: {
 ...
 *
 * Respuesta: {
 *   summary: string,                  // párrafo narrativo (2-4 oraciones)
...
export async function POST(req: Request) {
    try {
        ...
        // Formatear KPIs en un bloque legible
        ...
        const prompt = `Eres Kesito, consultor senior de retail. Vas a escribir un BRIEFING editorial
corto sobre lo que se ve en ${pageContext} ahora mismo.
...
3. Tono profesional pero humano. No corporativo acartonado.
4. Si hay algo que llame la atención (concentración alta, caída, anomalía),
   menciónalo con tono sutil.
5. NO uses bullets, NO uses encabezados, NO digas "¿quieres profundizar?".
6. Si los datos son neutros (ej: día normal), simplemente cuenta lo que hay
   sin inventar alarmas.

Determina también el TONO:
- "positive": Hay datos claramente positivos (crecimiento, números altos)
- "attention": Hay algo que requiere mirar (caídas, concentración, anomalía)
- "neutral": Día/período normal, sin destacar
