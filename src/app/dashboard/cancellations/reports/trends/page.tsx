...
const getStoreColor = (name: string) => {
    if (!name) return '#E11D48'; // Rose-600 for cancellations
...
export default function CancellationTrendsPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            let url = `/api/dashboard/cancellations/trends?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}&idTienda=${idTiendaParam}&groupBy=${groupBy}`;
    ...
    const storeColor = useMemo(() => {
        if (selectedStoreIds.length === 1) {
            ...
            return name ? getStoreColor(name) : '#E11D48';
        }
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        isActive ? 'bg-[#E11D48] text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-white'
                                    )}
                                >
                                    {label}
                                </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-4 rounded-none border border-slate-100 shadow-sm relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-3 opacity-10">
                        <Trash2 size={80} style={{ color: storeColor }} />
                    </div>
                    <div>
                    ...
                    <div className="flex justify-between items-center text-xs font-bold border-t border-slate-50 pt-4 mt-2">
                        <span className="text-slate-500">Promedio por Cancelación</span>
                ...
                <div className="bg-white p-4 rounded-none border border-slate-100 shadow-sm relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-3 opacity-10">
                        <ShoppingCart size={80} className="text-rose-500" />
                    </div>
                    <div>
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Cant. Cancelaciones</span>
                ...
                <div className="bg-white p-4 rounded-none border border-slate-100 shadow-sm relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-3 opacity-10">
                        {globalTrend > 0 ? <TrendingUp size={80} className="text-rose-500" /> : <TrendingDown size={80} className="text-emerald-500" />}
                    </div>
                    <div>
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Variación</span>
                        <h2 className={cn("text-2xl font-black mb-2", globalTrend > 0 ? "text-rose-600" : "text-emerald-600")}>
                            {globalTrend > 0 ? '+' : ''}{globalTrend.toFixed(1)}%
                        </h2>
                    </div>
                    <div className="flex justify-between items-center text-xs font-bold border-t border-slate-50 pt-4 mt-2">
                        <span className="text-slate-500">Vs. Periodo Anterior</span>
                        <span className={cn(globalTrend > 0 ? "text-rose-600" : "text-emerald-600")}>
                            {formatCurrency(currentTotal - prevTotal)}
                        </span>
                    </div>
                </div>

                <div className="p-4 rounded-none shadow-xl shadow-rose-500/20 relative overflow-hidden group" style={{ backgroundColor: storeColor }}>
                    <div className="absolute top-0 right-0 p-3 opacity-20">
                        <LayoutGrid size={80} className="text-white" />
                    </div>
                    <div>
                        <span className="text-[10px] font-black text-white/60 uppercase tracking-widest mb-1 block">Rendimiento Sucursal</span>
                        ...
                        <div className="p-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                             ...
                             {selectedStoreIds.length > 0 && (
                                <button 
                                    ...
                                    className="text-[9px] font-black text-[#E11D48] uppercase hover:underline"
                                >
                                    Limpiar
                                </button>
                             )}
                                 ...
                                 <div className="flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-none p-0.5 self-end md:self-auto">
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            metric === 'venta' ? 'bg-[#E11D48] text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-white'
                                        )}
                                    >
                                        <Trash2 size={12} /> Total
                                    </button>
                                    <button
                                        onClick={() => setMetric('operaciones')}
                                        className={cn(
                                            'flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest transition-all',
                                            metric === 'operaciones' ? 'bg-[#E11D48] text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-white'
                                        )}
                                    >
                                        <ShoppingCart size={12} /> Cant
                                    </button>
                                    <button
                                        onClick={() => setMetric('ticket')}
                                        className={cn(
                                            'flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest transition-all',
                                            metric === 'ticket' ? 'bg-[#E11D48] text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-white'
                                        )}
                                    >
                                        <Ticket size={12} /> Promedio
                                    </button>
                                </div>
