...
export default function PurchaseOrdersPage() {
    ...
    const fetchReceiptDetails = async (folioRecibo: string, shouldOpenModal = true) => {
        ...
        try {
            const res = await fetch(`/api/purchases/receipts/details?folioRecibo=${folioRecibo}`);
            const data = await res.json();
            if (data.error) {
                console.error('API Error:', data.error);
            } else {
                        ...
                        {filteredOrders.length > 0 ? (
                            ...
                            <div className="min-h-[600px] flex flex-col items-center justify-center text-center p-8">
                                ...
                                <p className="text-slate-500 max-w-md mx-auto">
                                    {loadingOrders 
                                        ? "Cargando órdenes de compra desde el buffer..." 
                ...
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white w-full max-w-6xl h-[85vh] shadow-2xl border border-slate-200 flex flex-col animate-in slide-in-from-bottom-4 duration-300">
                        ...
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white">
                            <div className="flex flex-col">
                                ...
                                <div className="flex flex-wrap items-center gap-3 mt-1">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{selectedOrderForKanban?.Tienda}</p>
                                    <div className="w-1 h-1 rounded-full bg-slate-300" />
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{selectedOrderForKanban?.Proveedor}</p>
                                    <div className="w-1 h-1 rounded-full bg-slate-300" />
                                    ...
                                    {receiptData?.header.UUID && (
                                        <>
                                            <div className="w-1 h-1 rounded-full bg-slate-300" />
                                            <a 
                                                href={`https://recursos.kykcloud.mx/${receiptData.header.UUID}`} 
                                                target="_blank" 
                        ...
                        <div className="flex bg-slate-50 border-b border-slate-100 px-6">
                            <button
                                onClick={() => setActiveReceiptTab('recibo')}
                                className={cn(
                            ...
                            {receiptData?.returnItems && receiptData.returnItems.length > 0 && (
                                <button
                                    onClick={() => setActiveReceiptTab('devoluciones')}
                                    className={cn(
                                        "px-6 py-3 text-[11px] font-black uppercase tracking-widest transition-all border-b-2",
                                        activeReceiptTab === 'devoluciones' 
                            ...
                            {loadingReceiptDetails ? (
                                ...
                                <table className="w-full text-left border-collapse bg-white shadow-sm border border-slate-200">
                                    ...
                                    <tbody className="text-slate-700">
                                        ...
                                        {((activeReceiptTab === 'recibo' ? receiptData?.receiptItems : receiptData?.returnItems)?.length === 0) && (
                                            <tr>
                                                <td colSpan={activeReceiptTab === 'recibo' ? 15 : 6} className="p-10 text-center">
                                    ...
                                    <tfoot className="bg-slate-50 border-t-2 border-slate-200 sticky bottom-0">
                                        <tr>
                                            <td colSpan={activeReceiptTab === 'recibo' ? 14 : 5} className="p-4 text-right text-[11px] font-black uppercase text-slate-500 tracking-widest">
                                                Total {activeReceiptTab === 'recibo' ? 'Recibido' : 'Devolución'}
                                            </td>
                                            <td className={cn(
                                                "p-4 text-right text-base font-black",
                                                activeReceiptTab === 'recibo' ? "text-emerald-600" : "text-rose-600"
                                            )}>
                                                {formatCurrency((activeReceiptTab === 'recibo' ? receiptData?.receiptItems : receiptData?.returnItems)?.reduce((acc, item) => acc + item.Total, 0) || 0)}
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
